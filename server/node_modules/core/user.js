...
class User {
	 ...
	 */
	setGroup(var1) {
		this._group = GroupManager.getInstance().getGroup(var1);
	};
	...
	notify(var1) {
		try {
			this._socket.send(JSON.stringify(var1));
		} catch(e) {};
	};
	...
	signin(var1) {
		...
		if(!(typeof var1.username === 'string' && typeof var1.password === 'string') || (var1.username === '' || var1.password === '')) {
			self.notify({
				action : "signin_failed",
				extra : {
					message : "translate:user/missing_signin_data"
				}
			});
	...
	signup(var1) {
		...
		var1.username = var1.username.replace(/[^A-z0-9-_]/g, "");

		if(!(typeof var1.username === 'string') || var1.username === '') {
			self.notify({
				action : "signup_failed",
				extra : {
					message : "translate:user/missing_username"
				}
			});
		...
		User.userExists({
			username : var1.username,
			callback : (err) => {
				if(err && !var1.password) {
					self.setLogged(true);
					self.setUsername(var1.username);
					self.setAvatar("1");
					self.setGroup('default');
					self.setPermissions(["send_message", "receive_message", "upload_file", "download_file"]);
					ChannelManager.getInstance().getDefaultChannel().addUser({
						user : self,
						force : false,
						callback : () => {}
				...
				if(!(typeof var1.password === 'string') || var1.password === '') {
					self.notify({
						action : "signup_failed",
						extra : {
							message : "translate:user/missing_password"
						}
					});
	...
	static addUser(var1) {
		...
		var1.on('message', (data) => {
			...
			switch(action) {
				...
				case "upload_init" :
					user.upload_init(parsed_data.extra);
					break;

				case "chart" :
					var1.send(JSON.stringify({
						action : "chart_success",
						extra : {
							users : self._users.length,
							data_amount : DownloadManager.getInstance().getDataAmount()
						}
					}));
					break;
				default:
	 ...
	 * @param {requestCallback} var1.callback
	 */
	static userExists(var1) {
		let username = var1.username.toLowerCase();
		let filepath = path.normalize(process.cwd() + '/server/users/' + username + '.json');

		fs.stat(filepath, (err, stats) => {
			if(err) {
				var1.callback.call(void(0), "translate:user/account_does_not_exists");
				return;
			}
